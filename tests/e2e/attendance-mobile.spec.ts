...
test.describe('mobile attendance clock-out regression', () => {
  ...
  test('preserves verified GPS location through WiFi selection before password clock-out', async ({ page, context }) => {
    let verifyClockPayload: Record<string, unknown> | null = null;
    ...
    await page.getByRole('button', { name: '下班打卡' }).click();
    await expect(page.getByRole('heading', { name: 'WiFi 驗證' })).toBeVisible();

    await page.getByLabel('Office-WiFi').check();
